...
class GaussianSLAM(object):
    ...
    def __init__(self, config: dict) -> None:
        ...
        self.mapping_frame_ids = frame_ids[:: config["mapping"]["map_every"]] + [
            n_frames - 1
        ]
        ...
        self.submap_using_motion_heuristic = config["mapping"][
            "submap_using_motion_heuristic"
        ]
        ...
        if self.submap_using_motion_heuristic:
            ...
            self.new_submap_frame_ids = frame_ids[
                :: config["mapping"]["new_submap_every"]
            ] + [n_frames - 1]
    ...
    def should_start_new_submap(self, frame_id: int) -> bool:
        ...
        if self.submap_using_motion_heuristic:
            if exceeds_motion_thresholds(
                self.estimated_c2ws[frame_id],
                self.estimated_c2ws[self.new_submap_frame_ids[-1]],
                rot_thre=50,
                trans_thre=0.5,
            ):
    ...
    def update_keyframe_poses(self, lc_output, submaps_kf_ids, cur_frame_id):
        ...
        for correction in lc_output:
            ...
            self.estimated_c2ws[min_id : max_id + 1] = (
                torch.from_numpy(correct_tsfm).float()
                @ self.estimated_c2ws[min_id : max_id + 1]
            )

        # last tracked frame is based on last submap, update it as well
        self.estimated_c2ws[cur_frame_id] = (
            torch.from_numpy(lc_output[-1]["correct_tsfm"]).float()
            @ self.estimated_c2ws[cur_frame_id]
        )
    ...
    def run(self) -> None:
        ...
        for frame_id in range(len(self.dataset)):
            ...
            if frame_id in [0, 1]:
                ...
                exposure_ab = torch.nn.Parameter(
                ...
                estimated_c2w, exposure_ab = self.tracker.track(
                    ...
                    torch2np(
                        self.estimated_c2ws[
                            torch.tensor([0, frame_id - 2, frame_id - 1])
                        ]
                    ),
                )
            ...
            if self.should_start_new_submap(frame_id):
                ...
                save_dict_to_ckpt(
                    self.estimated_c2ws[: frame_id + 1],
                    "estimated_c2w.ckpt",
            ...
            if frame_id == len(self.dataset) - 1 and self.config["lc"]["final"]:
                print("\n Final loop closure ...")
                self.loop_closer.update_submaps_info(self.keyframes_info)
                lc_output = self.loop_closer.loop_closure(
                    self.estimated_c2ws, final=True
                )
                if len(lc_output) > 0:
                    submaps_kf_ids = self.apply_correction_to_submaps(lc_output)
                    self.update_keyframe_poses(lc_output, submaps_kf_ids, frame_id)
            if self.enable_exposure:
                self.exposures_ab[frame_id] = torch.tensor(
                    [exposure_ab[0].item(), exposure_ab[1].item()]
                )

        np.save(
            f"{self.output_path}/est.npy",
            torch.linalg.inv(self.estimated_c2ws).cpu().numpy(),
        )
        save_dict_to_ckpt(
            self.estimated_c2ws[: frame_id + 1],
            "estimated_c2w.ckpt",
        ...
        if self.enable_exposure:
            save_dict_to_ckpt(
                self.exposures_ab, "exposures_ab.ckpt", directory=self.output_path
            )
